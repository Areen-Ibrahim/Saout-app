...
class PushNotificationsServiceCoach {
  ...
  static Future<void> sendNotificationsTo(String token, BuildContext context,
      String userId, String title, String body, String coachId) async {
  ...
  static Future<void> setupFirebaseMessaging(BuildContext context,
    ...
    await saveFCMTokenCoaches(coachId);
    ...
    messaging.onTokenRefresh.listen((newToken) async {
      if (newToken != null) {
        ...
        await saveFCMTokenCoaches(coachId, newToken); // احفظ التوكن الجديد
      }
    });
  ...
  static Future<void> saveFCMTokenCoaches(String coachId, [String? token]) async {
    final FirebaseFirestore firestore = FirebaseFirestore.instance;

    if (token != null) {
      // احفظ التوكن في قاعدة البيانات
      await firestore.collection("Coaches").doc(coachId).set(
          {"fcmToken": token}, SetOptions(merge: true));
      print(
          "FCM Token saved for user $coachId: $token"); // طباعة تأكيد حفظ التوكن
    } else {
      // احصل على التوكن الحالي
      String? currentToken = await FirebaseMessaging.instance.getToken();
      await firestore.collection("Coaches").doc(coachId).set(
          {"fcmToken": currentToken}, SetOptions(merge: true));
      print(
          "FCM Token retrieved and saved for coachId $coachId: $currentToken"); // طباعة تأكيد حفظ التوكن
        }
  }
}
