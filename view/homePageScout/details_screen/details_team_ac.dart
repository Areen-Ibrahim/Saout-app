...
import 'package:saoutapp/controllers/controller_scout/details_controller/team_details_controller.dart';
...
class DetailsTeamAc extends StatefulWidget {
  const DetailsTeamAc({super.key});

  @override
  State<DetailsTeamAc> createState() => _DetailsTeamAcState();
}

class _DetailsTeamAcState extends State<DetailsTeamAc> {
  ...
  final TeamDetailsController teamDetailsController = Get.put(TeamDetailsController());
  ...
  // متغير للتحكم في الـ PageView
  final PageController _pageController = PageController();

  // متغير حالة الصفحة النشطة
  RxInt activePage = 0.obs; // لجعلها قابلة للمراقبة
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: TextTitleAddPlayer(text: 'Academy Details',),
        iconTheme: IconThemeData(color: Colors.white, size: 20),
      ),
      body: FutureBuilder<Map<String, dynamic>>(
        future: teamDetailsController.getTeamDetailsWithPlayerInfo(teamId),
                      ...
                      children: [
                        ...
                        PopupMenuButton<int>(
                          ...
                          onSelected: (value) {
                            ...
                            } else if (value == 4) {  // إضافة زر "Request"
                          ...
                          itemBuilder: (context) => [
                            ...
                            PopupMenuItem<int>(  // زر الإرسال
                              value: 4,
                              child: Row(
                                children: [
                                  Icon(Icons.request_page, color: Colors.blue),  // أيقونة الطلب
                                  SizedBox(width: 8),
  ...
  void sendRequestToCoach(String coachEmail) {
    print("Sending request to coach at: $coachEmail");

    // هنا يمكن إضافة الكود الخاص بإرسال الطلب إلى المدرب
    // مثل إرسال طلب إلى API أو قاعدة بيانات
    // على سبيل المثال:
    // await apiService.sendRequest(coachEmail, teamId);

    // بعد إرسال الطلب بنجاح، يمكنك إضافة أي ردود فعل مثل طباعة رسالة تأكيد:
    print("Request sent successfully to coach.");
  }
